...
for($index=0;$index<count($data['cards']);$index++){
  ...
  if($yearOfCourse===2019){
    $getgroupid2019="SELECT `id` FROM `groups` WHERE `year`=2019 ";
  ...
  for($index1=0;$index1<count($data['cards'][$index]['tarefas']);$index1++){
    ...
    $sendIDgroup="INSERT INTO `students_to_group`(`group_id`)VALUES('$groupID2019')";
    mysqli_query($connection,$sendIDgroup);
  }
  }
